...
public class CategoriesController {
    ...
    @PostMapping
    public ResponseEntity<Map<String, String>> post(@PathVariable String conveyorCode, @Valid @RequestBody CategoryTransaction dto) {
        String normalizedConveyor = conveyorValidator.normalize(conveyorCode);

        log.debug("Received category import request: transactionId={}, categoryCode={}", dto.getId(), dto.getCategoryCode());
        ...
        try {
            ...
            log.info(
                "Category import event sent successfully: transactionId={}, categoryCode={}, key={}",
                dto.getId(),
                dto.getCategoryCode(),
                messageKey
            );
        } catch (Exception e) {
            log.error("Failed to send category import event: transactionId={}, categoryCode={}", dto.getId(), dto.getCategoryCode(), e);
            throw e;
        }

        return ResponseEntity.accepted().body(
            Map.of("status", "accepted", "transactionId", dto.getId(), "categoryCode", dto.getCategoryCode())
        );
    }
