...
@RestController
@RequestMapping("/api/v1/import/returns")
...
public class ReturnsController {
    ...
    @PostMapping
    public ResponseEntity<Map<String, String>> post(@Valid @RequestBody ChipReturnedTransaction dto) {
        log.debug("Received chip return import request: transactionId={}, chipCode={}",
        ...
        String topicName = topicConfig.buildRawTopic("returns");

        try {
            producer.send(topicName, messageKey, evt);
            log.info("Chip return import event sent successfully: transactionId={}, chipCode={}, key={}",
                    dto.getId(), dto.getChipCode(), messageKey);
        } catch (Exception e) {
            log.error("Failed to send chip return import event: transactionId={}, chipCode={}",
