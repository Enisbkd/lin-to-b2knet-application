...
@RestController
@RequestMapping("/api/v1/import/sizes")
...
public class SizesController {
    ...
    public ResponseEntity<Map<String, String>> post(@Valid @RequestBody ModelSizeTransaction dto) {
        log.debug("Received size import request: transactionId={}, modelCode={}, sizeCode={}",
        ...
        String topicName = topicConfig.buildRawTopic("sizes");

        try {
            producer.send(topicName, messageKey, evt);
            log.info("Size import event sent successfully: transactionId={}, modelCode={}, sizeCode={}, key={}",
                    dto.getId(), dto.getModelCode(), dto.getSizeCode(), messageKey);
        } catch (Exception e) {
            log.error("Failed to send size import event: transactionId={}, modelCode={}, sizeCode={}",
